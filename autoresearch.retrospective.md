...
Do not continue BY integration/plumbing experiments unless that primitive (or a successor) survives those charged hard-piece gates first, including the two-denominator objection. For the relaxed 3M question, the same accountant rule still matters: Strategy E deletes the second denominator algebraically, but with the current product-clean multiply it can afford only `911,490` CCX for the one DIV. Granting the best fixed-control replay `873,600` leaves just `37,890` for selector/parser/cleanup; measured decoder alone is `63,936` and lowword selector+decoder is `278,208`. Thus a <3M low-scratch BY/Strategy-E path still needs a real integrated one-denominator selector, not just a replay-body number.

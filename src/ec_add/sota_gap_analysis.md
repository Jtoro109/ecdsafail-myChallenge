...
| | Toffoli (PA) | qubits | per-Eea forward | status |
|---|---:|---:|---:|---|
...
From TRACE_PHASES:
```
Eea internals (kal_* + bk_*):            3.24M Toffoli (78%)
state_a_halve + state_b_double (scale correct):   207k Toffoli (5%)
state_a_mul1 + state_a_mul2 + state_b_mul + mul3:   239k Toffoli (6%)
sol_halve_tail:                                 8k (0%)
...
Each Eea (fwd + bwd measurement-uncompute) costs ~1.62M.
...
- 1 Eea fwd+bwd at ~1.3M (20% cheaper than ours, plus fewer iters)
...
1. **Cheaper per-iter Eea**: ~15n → ~10n CCX/iter. Removes ~30-35% of Eea.
2. **Lower iter count**: 407 → ~350 via a tighter convergence bound or windowed iters.

## What tricks close the gap

Concrete optimizations found in public literature that apply to Eea:
...
`mod_double_inplace_fast` costs ~n CCX (constant add for FastModulo
...
Trick: delay the FastModulo correction to a single bulk correction at the
end of Eea (like Kim 2026's "postponed modular reduction") —
...
Our pair iters = 407. Classical Eea termination probability → 1 at
...
Combine:
- Trick 3 (r-doubling FastModulo postponement): -38k
...
Additionally need a 33% cheaper per-iter Eea (15n → 10n). Only
achievable via:
...
If both are applied to Eea: per-iter cost drops from ~15n to ~6-8n.
...
across both Eeas (fwd+bwd). Fusion eliminates roughly half = **~660k
...
coupling means the `eea_iteration` call contract changes.

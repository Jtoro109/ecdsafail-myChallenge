...
current Eea's `u,v,r,s,m_hist` state.
...
Current Eea stores this as `(u,v,r,s)` plus history. In 600 scratch, the
only way to keep Eea-like inversion alive is to fold at least two of those
four n-bit roles into `tx,ty`.

A minimal Eea-like layout would have to look like:
...
history, the four n-bit Eea roles already exceed 600 scratch unless they
are folded into the data registers.

## 4. New structural idea: use Eea's coefficient transform on `ty`

Instead of treating Eea as an ancilla subroutine, seed its coefficient
register with the data value `dy`.

Use a canonical-mod-p coefficient version of Eea. For a fixed denominator
...
The test module `eea_linear_transform.rs` verifies empirically for the
...
This is the first genuinely low-qubit-looking Eea algebra found in this
...
run Eea backward, `ty` could be written to an arbitrary target value.
...
have enough information for `lambda`, but not enough to rewrite the Eea
...
4. **Run a tiny second coefficient transform, not a second full inversion.**
   ...
   or a short replay. This would be far cheaper than a full second Eea if
...
from jumped/windowed Eea instead.
...
`a_coefficient_cancellation_is_dense_on_toy_eea`, mask bits of the Eea
...
Eea invocation and fits in ~600 scratch, then point-add becomes:
...
Eea by seeding the coefficient register with `y`.

### Why a quotient-copy DIV does not fit 600 scratch

A tempting DIV implementation is:

1. Run Eea forward with `tx` as the denominator state and `ty` as the
...
3. Run Eea backward to restore/clean the Euclidean state.
...
backward output be `S1 = Ry + b·Qy`. The required Eea-coefficient update is
...
Eea's coefficient transform gives:
...
transform Eea. A viable design must be more radical: make `r` itself one
...
## 10. Output-register use of `r`: reduces to self-cleaning forward Eea

If Eea coefficient `r` is allowed to become final `ty`, we can avoid the
quotient-copy lower bound:

```text
start:   tx=x, ty=y, scratch r=0, scratch u=p
forward coefficient Eea using ty as s:
...
only if** forward Eea can be made self-cleaning, i.e. no persistent
...
So a self-cleaning forward Eea is not information-theoretically dead for
...
So a forward-only self-cleaning Eea cannot simply uncompute each branch by
...
encode branch history.  A self-cleaning Eea needs explicit history or a
genuinely different state invariant.

Recomputing branch history directly from the preserved initial `x` is also not
a tiny oracle.  `initial_x_to_branch_history_oracle_is_dense_on_toy_eea`
...
on-demand branch oracle is basically another Eea-like computation, not a
...
compression is not enough unless the rest of the Eea state is also
fundamentally folded.

A literature-inspired Kim/unconditional-Eea escape was checked too.  The
...
So exact history + coefficient-transform Eea cannot hit the 1175q low-qubit
...
### Jumped Eea matrix route hard gate

A separate two-inversion route was Eea windowing: store short matrix hints
and apply selected `t`-step matrices to `(u,v)` and `(r,s)`.  The arithmetic
intensity model already looked suspicious; `selected_matrix_variable_coeff_lower_bound_kills_hybrid_eea_windows`
...
QROM-selected Eea matrix application is dead with current arithmetic.  A
...
- before state_a Eea, set `ty := dy + dx`,
- compute tagged slope `-(λ+1)`,
- consume tagged `ty` to zero with the existing `state_a_mul2`,
...
because it still uses the old Bennett-clean Eea and m_hist. Its importance
is that the tag algebra works in the real circuit.

A stronger default-off implementation was then wired behind
`KAL_TAGGED_DIV_COEFF_CHANNEL=1`: while ordinary Eea is running, it carries
...
state_a's two schoolbook multiplications from the scaffold. It passes the real
...
- the external coefficient channel is live during Eea, adding a full
  256-qubit `lam` register at the Eea peak;
- each Eea iteration needs data-channel cswaps, a controlled modular add,
...
branch pair. This suggests a branch-only Eea generator can replace the full
...
1. run denominator Eea while recording `m_hist`, `a_hist`, and `add_hist`,
...
qubits      = 2,714
classical/phase/ancilla failures = 0
peak phase  = state_b Eea, not branch-DIV
```

This is a useful qubit-shape result: the tagged-DIV scaffold itself is now below
the current baseline peak, and state_b Eea again dominates peak qubits. It is
...
inversion for state_b cleanup:

```text
KAL_PAIR2_BRANCH_INV_ROLL=1 cargo run --release -- --note state_b-branch-inv-roll
...
expensive than the existing full Eea state. The extra live objects are
`m_hist+a_hist+term_idx`, an explicit `(inv_raw, coeff_s)` pair, and the
FastModulo cmod-add transients during replay. Therefore **branch-history replay is
not a general compact replacement for Eea inversion**; it only makes sense
...
A one-pair Eea DIV folded into the input registers uses `tx` as `v` and
...
`scratch600_sidecar_tag_bits_do_not_fix_eea_branch_recovery` evolves an
...
slack.  So under 600 scratch, a Eea-like one coefficient pair plus a small
...
The complementary toy check `unreduced_coefficient_eea_self_cleans_but_width_kills_scratch600`
...
So unreduced coefficients are a conceptual self-cleaning Eea, but not a
...
poststate collisions are rare: `curve_restricted_tagged_eea_poststate_ambiguity_is_small_but_not_exact`
...
poststate inverse still has generic ambiguity: `secp_curve_support_does_not_make_eea_branch_choice_locally_free`
...
`one_lane_linear_sidecar_has_no_closed_update_for_all_eea_branches` finds no
...
1. **End-state branch predicate synthesis**: derive a reversible predicate for
   ...
   checks per iter, forward-only DIV may still beat the 2-Eea scaffold.
   If it costs a full inverse-step replay or many n-bit comparisons, kill it.

2. **Direct DIV synthesis**: ignore current Eea structure and design a
...
4. **Cost if successful**:
   - one DIV/Eea-like invocation: target ~1.6M or less
   - delete `state_a_mul1`, `state_a_mul2`, second Eea: save ~1.7M
...
This is now the main ground-up research direction alongside jumped Eea.
...
already-measured state_b scaled inverse/product-clean primitive.  The budget test
...
would also solve the earlier state_b cleanup obstruction; without it, there is no
...
A different one-Eea cleanup idea is to preserve enough old denominator data
...
## 12. Attempt F: absorb Eea's scale by pre-scaling the denominator

Eea exposes a raw coefficient of the form

```text
inv_raw = -v^-1 * 2^iters  (mod p)
```

The ordinary point-add corrects this by applying `iters` halvings to the state_a
slope and `iters` doublings before the state_b cleanup.  Those two correction
loops cost about **206k Toffoli** total, so a natural idea is to feed Eea a
scaled denominator
...
sites when the prescaler used exact RippleAdder arithmetic:
...
The folded variants write the scaled denominator directly into Eea's `v_w`
...
## Direct controlled-constant FastModulo correction note
...
The helper `csub_nbit_const_direct_fast` avoids loading the sparse FastModulo
...
The Eea `r` smallness shortcut now defaults to 257 iterations instead of
255:

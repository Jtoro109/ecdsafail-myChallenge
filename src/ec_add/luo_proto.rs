...
//!   2. compare it to our current Eea scaffold,
...
/// - tx,ty = 2n = 512
/// - Eea persistent state = u,v_w,r,s,m_hist,f ≈ 4n + iters + 1
...
/// Conservative point-add budget if we swapped our Eea block for a Luo/PZ
/// block *without* changing anything else in the affine scaffold.
fn naive_luo_ec_add_budget_conservative(n: usize, current_other_peak: usize) -> usize {
...
fn naive_luo_ec_add_budget_optimistic(n: usize, current_other_peak: usize) -> usize {
    let tx_ty = 2 * n;
...
mod tests {
    ...
    fn luo_budget_is_qubit_relevant() {
        ...
        let luo_cons = naive_luo_ec_add_budget_conservative(n, current_other_peak);
        let luo_opt = naive_luo_ec_add_budget_optimistic(n, current_other_peak);
    ...
    #[test]
    fn luo_pz_gate_slope_is_not_ec_add_sota_shaped() {
        ...
        let per_ec_add_proxy = luo_whole_ecdlp_toffoli / (2 * n);
        let google_low_qubit = 2_700_000usize;
        let google_low_gate = 2_100_000usize;
        eprintln!(
            "Luo/PZ gate-slope proxy: whole_ecdlp={luo_whole_ecdlp_toffoli}, per_ec_add={per_ec_add_proxy}, ratios_vs_google=({:.2}, {:.2})",
            per_ec_add_proxy as f64 / google_low_qubit as f64,
            per_ec_add_proxy as f64 / google_low_gate as f64
        );
        assert_eq!(per_ec_add_proxy, 31_981_568);
        assert!(per_ec_add_proxy > 10 * google_low_qubit);
        assert!(per_ec_add_proxy > 15 * google_low_gate / 1); // loose integer guard
    }
    ...
    fn optimistic_luo_still_needs_hundreds_more_qubits_cut() {
        ...
        let cur = current_budget_estimate(n, 407);
        let target_total = 512 + 600;
        let luo_opt = naive_luo_ec_add_budget_optimistic(n, cur.lambda_and_mul_state);
        eprintln!(

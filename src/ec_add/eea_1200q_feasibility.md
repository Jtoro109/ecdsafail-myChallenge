# Deep research: Can Eea fit in a 1200–1300 qubit budget?

Scope: decide whether the Eea-based inverter is architecturally viable at
Google's low-qubit operating point (1175–1425q total for the whole
point-add), or whether closing the SOTA gap requires replacing Eea.

Conclusion up front (derived below, and with one classically-verified claim):

> **Eea CAN fit in ~1200q, but only after three simultaneous structural
...
> `single_inv_numeric.rs`) — saves one whole Eea pass; (3) **r-into-next-
> multiplier fusion** — saves **256q**. All three together land the
> single-inversion Eea peak at **~1050–1200q** depending on transient
> handling, at an estimated +0–20% Toffoli cost.
>
> **Without any one of the three, Eea at 1200q is infeasible.** If we
> insist on two full inversions, Eea is dead and we have to replace it
> — but no public non-Eea inverter (B-Y, Fermat, Chevignard, Luo,
> Kim wide-r) beats Eea at the 1200q / few-M-Toffoli corner.
>
> **Recommendation: keep Eea. Attack m_hist elimination next.**
...
| lam (slope λ)                         |    256 | live across both Eea passes |
| Eea u                             |    256 | shrinks but alive full pass |
| Eea v_w                           |    256 | shrinks but alive full pass |
| Eea r                             |    256 | grows from 0; alive full pass |
| Eea s                             |    256 | grows from 0; alive full pass |
...
| **persistent subtotal**               | **2197** | Eea alive set |
| transient (step 4 tmp + RippleAdder carries + const loads) |    ~520 | bursts peak |
...
survive (256q), only **432 qubits remain** for Eea state AND transient
bursts. Current Eea persistent = 1685q. **Gap: 1253 qubits to eliminate.**

This is the number we are trying to beat. Call it `Δ = 1253`.

---

## 2. The six Eea registers: what each is, and how compressible it is
...
second half of Eea only needs *one* n-register to hold `(u,v_w)` fused.
But the *first* half needs two. The 256q saving is real only at the tail
of Eea, which doesn't dominate peak.
...
1. **Recompute instead of store** (Bennett pebbling). If we re-simulate
   ...
   full Eea cost to recover each bit. With n/k pebble depth we get a
   Bennett tradeoff: `recompute_cost ≈ n^{log_k(2k-1)}` vs `qubit_cost ≈ k · n/log`.
   For k=2, sqrt(n) space-time Eea: ~16× wall-clock, ~50q. **Saves ~357q.**
...
6. **Stream-measured with classical repair** (novel): measure m_i as it's
   ...
   **Critical question**: does m_i, as emitted from the current eea
   body, have coherent superposition components? Answer from classical
   trajectory analysis: **No.** The Eea algorithm is classically
...
7. **Recompute m_i in-circuit at backward-start from the live Eea state.**
   This was the key question of §7 below. Since backward Eea is the
   ...
   **CLASSICALLY VERIFIED** (this session): see
   `eea_classical_replay.rs`. On 256,000 (input, iter) samples:
...
### 2.4 Summary of raw Eea register compression
...
Note: 1025 is the absolute information-theoretic floor for classical Eea
...
exactly), so `free_vec` is safe. **Implementation cost: refactor Eea
...
- `u` at iter `i` (backward): width `= 2n − i + 1 = iter+1` (backward counts
  ...
  *equivalent* forward iter was `i = 2n − 1 − j`. Invariant: bitlen(u) ≤ 2n − i.
  ...
  (From trace: peak at state_a_mul1 = 2716 is driven by the mul's
  FastModulo-reduction const-add transient. It's NOT Eea's persistent
  state, it's Eea's **persistent state + mul transient** because mul
  happens between passes with lam alive.)

  Actually the peak site is `state_a_mul1`, which is *between* the Eea
  passes but with Eea state still alive (for backward). So Eea
...
**Revised**: dynamic shrink saves qubits *during* Eea body but doesn't
help at state_a_mul1 because Eea body isn't running there. **Peak drop
...
Here's the lower bound argument for Eea, put rigorously.

At the moment of backward Eea's first iteration, we need to know:
...
**Plus**: the bk_bulk_step4 transient of tmp (n) + RippleAdder carries (n−1)
≈ 2n ≈ 512 extra qubits during its body.

**Theoretical Eea peak: ≥ 4n + 2n = 1536 qubits for backward body alone.**
Plus tx, ty, lam if they're still alive = 1536 + 768 = 2304q.

This is a rigorous lower bound for Eea-with-standard-mul-primitive-transients.
**It says Eea in the current architecture cannot go below ~2300q if
the inverter runs while λ and ty are live.**

If we sequence things so λ, ty aren't alive during backward Eea, and
...
2. Eliminating the RippleAdder transient (saves n−1 → step 4 transient down to ~n)
3. Reusing tx as one of Eea's registers (saves 256q)
...
- Do ONE Eea on `c = dx · dy_correction_term`
- Derive `dx⁻¹` and `(Rx−Qx)⁻¹` both from c⁻¹ by in-circuit multiplications

One Eea pass + three in-place multiplications. Total persistent qubits
during the inversion:

| slot                     | qubits | |
|--------------------------|-------:|--|
| tx (acts as input register to Eea) | 256 | reused |
| ty                       |    256 | idle during Eea — CAN reuse as Eea s! |
| scratch for c            |    256 | |
| Eea u                |    256 | if ty not reused, -256 |
| Eea v_w              |    256 | |
| Eea r (output)       |    256 | |
| Eea s                |      0 | if ty reused |
| m_hist                   |    407 | |
| flags                    |      1 | |
| **subtotal if ty reused as s** | **1688** | |

Plus step 4 transient (tmp + carries) = +512 peak.
**Total: ~2200q during backward Eea body.**
...
- `λ` is not alive during Eea body (we compute Eea, then multiply
...
**Revised floor for modified-Eea single-inversion**:

| slot              | qubits |
|-------------------|-------:|
| tx                | 256 (reused as c-input then output) |
| ty                | 256 (reused as Eea s or similar) |
| u                 | 256 |
| v_w               | 256 |
...
**1609q floor for a fully-optimized single-inversion Eea point-add.**

**Still 400q above the 1200q target.** And note: ty reuse requires an
architecture where ty is genuinely not needed during Eea — which is
...
But this brings lam (256q) back into alive-set during backward Eea of
the SECOND inversion... which we've eliminated in single-inversion. OK.

**Verified: 1609q is the aggressive Eea single-inversion floor.**

Google 1425q low-gate fits with further tricks I cannot identify. Google
1175q requires techniques substantially beyond Eea.
...
1. **Windowed Eea (jump-divstep)**: process multiple iterations at once
...
2. **Eliminate r entirely via in-place accumulation into tx**: at end of
   Eea, r = c⁻¹ · 2^{2n}. If we can arrange for the FastModulo rescale
   and the subsequent multiplication by ty to happen INSIDE the Eea
...
3. **Elimination of m_hist entirely** via a structurally reversible
   iteration (not Kim). The HRSL Fig 6b swap-based Eea has this
...
iterations each (see `eea_classical_replay.rs`).
...
- **Backward**: at the start of each backward iter, the live Eea state
...
### 7.3 Remaining feasibility gates for 1200q Eea
...
(A) **Single-inversion scaffold** — Strategy C in
    ...
    *algebra* is proven; the *reversible circuit* needs to be written.
    ...
    Eea? One idea: express `c_inv` as `(dx · N)^{-1}` and re-derive
...
## 8. Comparison to replacing Eea

If Eea is kept:
- Best case (§7): 1200q, +30% Toffoli from HRSL structural change + single-inv
  windowed mul savings. Projected: 1200q / ~5–6M Toffoli. **Eea viable.**
...
**No published non-Eea inverter beats Eea at the 1200q /
few-M-Toffoli corner.** Conclusion: **keep Eea, attack the HRSL +
single-inversion + r-elimination combination.**

---

## 9. Proof sketch: why 1200q is impossible without structural Eea changes

Claim: a single-inversion point-add using our current Eea (with
...
Proof:
- Persistent state during backward Eea body: `{tx, v_w (was u), r,
...
- Step 4 transient: tmp (n qubits) + RippleAdder carries (n qubits) = 2n.
...
2. **Second milestone**: single-inversion point-add scaffold using the HRSL
   eea. Proves end-to-end reversibility at < 1500q peak.
...
the 1200q target requires non-Eea inversion.

---

## 12. Honest limitations of this analysis

- The 2n-iter upper bound on Eea (hence m_hist = 2n bits) might be
  reducible to ~1.4n with amortized analysis (Eea terminates much
  earlier than 2n in practice). Could shrink m_hist from 407 → ~350
  without Kim. Minor help.

- The "1609q floor" assumes the multiplier's internal FastModulo transients
  are negligible. They're not — a FastModulo mod-add with classical constant
...
- The 1156q projection assumes ty can be reused as Eea s. This is
...
inputs in `eea_classical_replay::tests::extract_fmin_truth_table`:
...
`eea_classical_replay::tests::verify_minimal_formula`:

...
pub fn iadd_dirty_2clean_qoffset(
    ...
    if n <= 4 {
        panic!("iadd_dirty_2clean_qoffset: n<=4 not supported yet, use ripple_adder_add");
    }
    assert!(q_dirty.len() >= n - 2, "need n-2 dirty qubits");
    let q_dirty = &q_dirty[..n - 2];
...
/// NARROW quantum-offset vented streaming add: `q_target += q_offset` where
/// `q_offset` has width `m <= n` and is treated as zero-extended to width `n`
/// (high bits `m..n` are constant |0>). This is a faithful specialization of
/// `add_vented_2clean_qoffset`: every gate that touched a high (zero) offset
/// bit is dropped (CX(0,*) = no-op), so the high part becomes pure carry
/// propagation. Requires `m >= 1`.
pub fn add_vented_2clean_qoffset_narrow(
    b: &mut B,
    q_target: &[QubitId],
    q_clean2: &[QubitId; 2],
    q_offset: &[QubitId],
    ...
    let n = q_target.len();
    let m = q_offset.len();
    assert!(m <= n, "narrow offset width must be <= target width");
    assert!(m >= 1, "narrow qoffset expects m>=1");
    if n == 0 {
        return;
    }
    if n == 1 {
    ...
    // broadcast_cx over the live (low m) offset bits only.
    for k in 0..m {
        b.cx(q_offset[k], q_target[k]);
    }
    ...
    for k in 0..n - 1 {
        ...
        if k == 0 {
            ...
            if k < m {
                ...
                // q_offset[k] == 0: CCX(q_target[k], cur, next).
                b.ccx(q_target[k], cur, next);
            }
        }
...
/// NARROW quantum-offset version of `xor_right_shifted_carries_into_qoffset`.
/// `q_offset` has width `m`; bits `>= m` are treated as zero.
pub fn xor_right_shifted_carries_into_qoffset_narrow(
    b: &mut B,
    q_src: &[QubitId],
    q_offset: &[QubitId],
    q_dst: &[QubitId],
    ...
    let off = |k: usize| -> Option<QubitId> {
        if k < m {
            Some(q_offset[k])
        } else {
            None
    ...
    for k in (1..n).rev() {
        ccx_with_qxor(b, q_src[k], off(k), q_dst[k - 1], None, q_dst[k]);
    }
    // broadcast over live offset bits only.
    for k in 0..m.min(n) {
        b.cx(q_offset[k], q_dst[k]);
    }
    // k=0 special (m>=1 so off[0] live).
    b.cx(q_offset[0], q_src[0]);
    if carry_in {
        b.x(q_offset[0]);
    }
    ...
    for k in 1..n {
        ccx_with_qxor(b, q_src[k], off(k), q_dst[k - 1], off(k), q_dst[k]);
    }
}

/// NARROW quantum-offset version of `iadd_dirty_2clean_qoffset`:
/// `q_target += q_offset + cin` with `q_offset` width `m <= n` (zero-extended),
/// using 2 clean + (n-2) dirty ancilla. The dirty qubits are borrowed and
/// restored exactly. This is the peak-reducing primitive for adding a short
/// (e.g. 22-bit) quantum spill into a wide register without allocating a
/// full-width padding register.
pub fn iadd_dirty_2clean_qoffset_narrow(
    b: &mut B,
    q_target: &[QubitId],
    q_dirty: &[QubitId],
    q_clean2: &[QubitId; 2],
    ...
    if n <= 4 {
        panic!("iadd_dirty_2clean_qoffset_narrow: n<=4 not supported, use ripple_adder_add");
    }
    assert!(q_dirty.len() >= n - 2, "need n-2 dirty qubits");
    let q_dirty = &q_dirty[..n - 2];
    ...
    add_vented_2clean_qoffset_narrow(
        b,
        q_target,
        q_clean2,
        q_offset,
    ...
    xor_right_shifted_carries_into_qoffset_narrow(
        b,
        &q_target[..n - 1],
        q_offset,
        q_dirty,
...
pub fn ciadd_dirty_3clean_qoffset(
    ...
    if n <= 4 {
        panic!("ciadd_dirty_3clean_qoffset: n<=4 not supported yet, use ripple_adder_add");
    }
    assert!(q_dirty.len() >= n - 2, "need n-2 dirty qubits");
    let q_dirty = &q_dirty[..n - 2];
...
mod tests {
    ...
    #[test]
    fn test_cisub_dirty_eea_pattern() {
        // Test with dirty qubits in Eea-specific patterns.
        let n = 256;
        let c_low = 0x1_0000_03D1u64;
        let trials = 50;
        let mut hasher = Shake256::default();
        ...
        for _trial in 0..trials {
            ...
            // Eea pattern: dirty[0] = u[0]=1 (at termination). Rest = 0.
            if dirty_u_lsb {
            ...
            if got == expected && dirty_ok && phase == 0 && ctrl_preserved {
                ...
                if bad < 3 {
                    eprintln!(
                        "cisub eea FAIL t={:#x} d0={} ctrl={} got={:#x} exp={:#x} d_ok={} phase={} ctrl_ok={}",
                        target, dirty_u_lsb, ctrl_val, got, expected, dirty_ok, phase, ctrl_preserved
                    );
                }
            }
        }
        assert_eq!(bad, 0, "eea pattern cisub: {ok}/{trials} passed");
    }

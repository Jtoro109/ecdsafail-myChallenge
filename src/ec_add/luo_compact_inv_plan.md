...
## Goal
Reduce Eea state from **1432q (4n + iters + 1)** to **~560q (3n + O(log n))**
...
The standard Eea keeps 4 n-bit registers `u, v, r, s` live throughout
...
**Is sum(bitlen(u), bitlen(v)) ≤ n throughout?** Eea invariant:
...
Luo's insight (§3.2): in the CLASSICAL execution, after each Eea
...
- Each iter's operations are CONTROLLED BY LENGTH REGISTERS (not direct
  ...
  fixed-position.
- FastModulo reduction gets complex (needs location-dependent fold).
...
4. Wire through `eea_forward` / `_backward` behind env var
   `KAL_LUO_COMPACT=1`.

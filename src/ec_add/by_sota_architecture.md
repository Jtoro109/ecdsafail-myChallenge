...
This is the current non-micro route.  Threshold tuning and local FastModulo/Eea
polish cannot close a ~1.4M Toffoli gap.  A SOTA-shaped exact affine point-add
has to delete the two current Eea-sized inversion objects and replace them
with a compact in-place DIV / product-clean DIV primitive.

## Target architecture

Use a scaled Bernstein–Yang tagged-DIV microprogram instead of Eea:
...
4. Delete state_a Eea and state_a schoolbook numerator multiplications; use the
   product-clean inverse shape for state_b cleanup.
...
```text
non-inversion affine scaffold after deleting state_a muls: ~0.64M
...
Do not spend more time on local Eea thresholds.  The next useful work is to

//! Ground-up structural probe: use Eea's coefficient update as a linear
...
//! - use `ty` as Eea's coefficient register `s`, initialized to `dy`,
//! - run a canonical-mod-p coefficient version of Eea.
//!
//! If this worked naively, the forward Eea would turn `ty=dy` into
//! `s=0` while `r = raw_inv(dx) * dy`, i.e. the scaled slope. Then Eea's
...
fn toy_branch_sequence_for_a_coeff(x: u64, p: u64, iters: usize) -> Vec<Branch> {
    ...
    for _ in 0..iters {
        ...
        if br.add {
            assert!(v >= u, "Eea branch should subtract smaller from larger");
            v -= u;
        }
        v >>= 1;
        if br.a_swap {
...
#[test]
fn initial_x_to_branch_history_oracle_is_dense_on_toy_eea() {
    ...
    // Eea.  So on-the-fly branch regeneration from x is equivalent to
    // rerunning a Eea-like computation, not a tiny lookup/phase gadget.
    let cases = [(4usize, 13u64), (6, 61), (8, 251), (10, 1021), (12, 4093)];
    for &(n, p) in &cases {
        let (degree, density) = toy_branch_history_phase_anf_stats(n, p);
        let table = 1usize << n;
        eprintln!(
            "toy Eea branch-history phase from x: n={n}, p={p}, degree={degree}, density={density}/{table}"
        );
        assert!(degree + 1 >= n);
...
#[test]
fn a_coefficient_cancellation_is_dense_on_toy_eea() {
    ...
    // data-dependent circuit.  On toy Eea transforms, mask bits of a(x) are
    // already full-degree and near-half-density ANFs.  So cancelling a(x) is not
    // a tiny phase/kickmix correction; it is effectively another Eea-like
    // branch computation.
    let cases = [
    ...
    for &(n, p, mask) in &cases {
        ...
        let table = 1usize << n;
        eprintln!(
            "toy Eea a(x) phase: n={n}, p={p}, degree={degree}, density={density}/{table}"
        );
        assert!(degree >= n - 1);
...
fn end_state_needs_coefficient_registers_to_recover_branch() {
    // A forward-only low-qubit DIV would like to run Eea without storing
...
#[test]
fn secp_curve_support_does_not_make_eea_branch_choice_locally_free() {
    // Curve support is information-theoretically useful (see the toy collision
    // test below), but it is not locally visible to the Eea poststate.  If
    ...
    eprintln!(
        "secp curve-supported local Eea branch candidates: hist={hist:?}, ambiguous={ambiguous}/{total}, frac={frac:.6}"
    );
...
#[test]
fn curve_restricted_tagged_eea_poststate_ambiguity_is_small_but_not_exact() {
    ...
    // 600-scratch-shaped crack in the no-history Eea wall, because exact
    ...
    for &(n, p) in &cases {
        ...
        eprintln!(
            "curve-restricted tagged Eea ambiguity: n={n}, p={p}, support={support}, states={states}, ambiguous_keys={amb_keys}, ambiguous_occurrences={amb_occ}/{total}, frac={frac:.6}, exact_sidecar_bits={sidecar}"
        );
        if n == 14 {
            println!("METRIC curve_restricted_eea_ambiguity_frac_n14={frac:.6}");
            println!("METRIC curve_restricted_eea_sidecar_bits_n14={sidecar}");
        }
        assert!(
            frac < last_frac || n == 10,
...
fn curve_collision_event_oracle_is_dense_in_natural_input_frame() {
    ...
    // "does tagged Eea on (dx,dy) ever hit a curve-supported ambiguous
...
#[test]
fn one_lane_linear_sidecar_has_no_closed_update_for_all_eea_branches() {
    // Could we compress the two-lane 2-adic sidecar to one b-bit register h and
    // update it locally under every Eea branch?  For any linear h=αr+βs,
...
fn implementable_curve_sidecar_still_extrapolates_over_88q_slack() {
    ...
    // branch operations.  This is far better than generic Eea history, but
    ...
    // slack left by folded one-pair Eea.  Only a strongly sublinear or
    ...
    assert!(
        ...
        "simple sidecar would fit 88q slack; revisit folded Eea"
    );
}
...
fn rolling_hash_sidecar_is_state_small_but_decoder_dense() {
    ...
    // hash of the Eea branch stream.  The update can be made essentially
    ...
    for &(n, p, bits) in &cases {
        ...
        eprintln!(
            "rolling hash Eea sidecar: n={n}, p={p}, bits={bits}, conflicts={conflicts}, states={states}, total={total}, support={support}"
        );
        assert_eq!(
            conflicts, 0,
...
fn measuring_unreduced_coefficient_high_bits_has_dense_phase() {
    ...
    // the Eea step locally reversible, then X-measure the high quotient bits
...
#[test]
fn unreduced_coefficient_eea_self_cleans_but_width_kills_scratch600() {
    ...
    for &(n, p) in &cases {
        ...
        if n == 8 {
            println!("METRIC unreduced_eea_max_coeff_bits_n8={max_bits}");
...
#[test]
fn scratch600_sidecar_tag_bits_do_not_fix_eea_branch_recovery() {
    ...
    // a Eea-like one-pair DIV already needs scratch for u and r: 2n=512
    ...
    // "one coefficient pair + small tag" Eea layout under a 600q scratch cap.
    let cases = [
        (4usize, 13u64, 3usize),
        (5, 31, 4),
        (6, 61, 5),
    ...
    for &(n, p, expected_min) in &cases {
        ...
        if n == 8 {
            ...
            println!("METRIC scratch600_bare_eea_div_scratch=512");
...
fn exhaustive_toy_full_poststate_does_not_recover_forward_branch() {
    ...
    // known.  A forward-only self-cleaning Eea therefore needs more than
...
fn any_x_only_additive_tag_is_only_a_bijection_not_branch_history() {
    ...
    // encode the Eea branch history.  Exhaustive toy checks with nonlinear
...
fn zero_coefficient_seed_loses_branch_information() {
    ...
    // self-cleaning forward-only Eea needs either an additional nonzero

...
- However, this is **not enough** to save plain B-Y: the per-iter reversible
  cost is still too high relative to Eea.

## Deliverable 2 (algorithm-space survey) — corrected final version

### 1. Eea almost-inverse (baseline)
- Classical ref: Burton S. Eea Jr., “The Montgomery inverse and its
...
- Conclusion: still worse than Eea.
...
- Quantum / reversible refs: effectively same family as RNSL/HRSL Eea.
- Conclusion: not a distinct win over Eea in our setting.
...
- Still potentially interesting as novel research, but now less grounded than
  a compressed Eea-local batching route, because we have exact empirical
...
A remaining risk in the hybrid Eea-jump idea was that even if the `(u, v)`
...
This removed the biggest remaining objection to the hybrid Eea-jump
moonshot.
...
**Conclusion: `hybrid Eea-jump` is still the bet, but the best first
...
4. do one ordinary final Eea micro-step,
...
## New classical proposal: exact 3-step hybrid Eea core

### Model
Standard Eea / binary almost-inverse update on `(u, v)` has four branch
cases:
...
|---|---:|---:|
| baseline 3 Eea iterations | 4,647 | 23,199 |
...
for the staged exact-3 skeleton, still much better than 3 ordinary Eea
...
This is **worse** than 3 ordinary Eea iterations (`4,647 CCX`).
...
| baseline 3 Eea iterations | **4,647** | **23,199** | **1,543** |
...
- `eea_forward` now has an experimental path gated by
  `KAL_BULK3_EXPERIMENT=1` that uses the specialized bulk primitive for the
  first **3** iterations,
- `eea_backward` uses a matching specialized backward for those same 3
  iterations,
...
- the isolated Eea inverse identity,
- the state_a block,
- the state_a+state_b block,

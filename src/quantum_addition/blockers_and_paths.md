...
**Our current circuit performs 2 full Eea invocations (forward+backward
per invocation = ~1.61M Toffoli per invocation).**

Each Eea invocation needs both a forward and backward pass for
reversibility. In our circuit: 2 × 1.61M = 3.22M Toffoli from Eea
...
2. **Approximate correctness** relaxes "exactly zero" to "approximately
   zero" (≤10⁻⁷ per invocation). Then partial Eea backward or
...
Forward Eea total = 802k CCX over 407 iters = 1970 CCX/iter average.
...
**So per-iter Eea forward cost ≈ 1600 CCX/iter for step 4 alone.**
...
3. **Coset representation** on the modular portion of step 4's
   ...
   But coset breaks comparators, which the Eea body relies on.

4. **Windowed-Montgomery style** replacement for Eea body. Litinski
   ...
   already. Not obviously a Eea win.
...
5. **Eliminate m_hist via recomputation** (verified classically in prior
   ...
   at state_a_mul2: -28k Toffoli.** Total impact maybe 1-2% of Toffoli,
   large qubit impact.
...
of cswaps implements the conditional branching of Eea (do-op-on-
...
## Blocker 4: 4×n Eea register layout

Our Eea state: u, v_w, r, s = 4n = 1024 qubits. Plus m_hist (407)
+ other ancillae. Total state during body: ~1432 persistent.

With data registers tx, ty (512q) live simultaneously, we're already
at 1944q persistent during state_a body. Add ~260q transient (step 4
tmp + RippleAdder) = 2204q. Measured peak 2716q (extra ~500 from lam_inner
and other concurrent state).

For 1300q target: need total non-data ≤ 788q. Means Eea state must
...
2. **Fuse (u, v) and/or (r, s)** when their combined bitlen fits in
   n bits. Per Eea invariant: bitlen(u) + bitlen(v) ≤ 2n − iter,
   so at iter ≥ n they fit in n. Saves ~n qubits in second half of
   pass. Doesn't help peak (which is at mid-pass).

3. **Reuse tx as v_w**: tx starts holding dx = v, which Eea reads
   into v_w via CX-copy. If we skip the copy and use tx directly as
   v_w, save 256q. After Eea, tx is back to holding dx (by
   Eea's invariant at termination). **Feasible, unexplored.**

4. **Fold r onto tx at end**: after Eea, we want tx to hold the
   inverse (or a derived value). If we don't allocate a separate r
   register but compute into tx directly, save 256q. **Requires
   restructuring Eea internal layout.**

Combined (1+3+4): saves 407 + 256 + 256 = 919q from current 2716.
Projected peak: **~1800q**. Still above 1300.

5. **Share with ty**: ty is idle during the inversion body (before state_a
   body closure reads it). If ty doubles as Eea s register, save
   another 256q. **Requires scheduling care.**

Combined (1+3+4+5): saves 1175q. Projected peak: **~1540q**. Closer.

6. **Windowed Eea** (small-window r, s): if r, s fit in ~n/w bits
...
Our Eea uses `u > v_w` (step 2) and `v_w == 0` (step 0) — both
comparators. **Directly porting to coset breaks Eea.**
...
3. **Restructure algorithm** to avoid comparators. B-Y divsteps is
   comparator-free — but we've shown it's worse for Eea.

## Path evaluation

Paths in rough order of (Toffoli impact × feasibility):

### Path A: coset rep for non-Eea mod-adds
Apply coset ONLY to operations outside Eea body (the muls, halves
in state_a_halve/state_b_double). No comparators to worry about. Savings:
...
non-Eea parts; bridge to/from computational basis at Eea entry/exit.

### Path B: eliminate m_hist + karatsuba-1 at state_a_mul2
Known unlock from prior sessions. -407q persistent, -28k Toffoli.
Straightforward but has phase-correction complexity.

### Path C: Eea per-iter reduction via step 3+9 cswap optimization
Highest-potential Toffoli target but structurally hardest. Requires
inventing a new Eea variant. Not in public literature.
...
2. **Path A** (coset for non-Eea ops) is medium-risk, moderate

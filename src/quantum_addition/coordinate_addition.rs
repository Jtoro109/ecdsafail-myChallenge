...
//!
//! ## Key takeaway so far
//! Plain B-Y (`w = 1`) is still worse than Eea on raw iteration count.
...
/// Env-gated smoke output used by `src/ec_add/mod.rs` when BY_TEST=1.
...
fn add_signed_shifted_term_naf_freepos_scaffold(
    b: &mut super::B,
    src: &[super::QubitId],
    dst: &[super::QubitId],
    shift: usize,
    ...
    if subtract {
        super::sub_nbit_qq_fast(b, &addend, &dst_slice);
    } else {
        super::add_nbit_qq_fast(b, &addend, &dst_slice);
    }
    if !src.is_empty() {
        let sign = src[src.len() - 1];
        for i in copy_len..len {
...
fn signed_coeff_naf_terms_for_scaffold(coeff: i128) -> Vec<(usize, bool)> {
    let coeff_is_negative = coeff < 0;
    ...
    while mag != 0 {
        ...
        let positive_digit = (mag & 3) == 1 || mag == 1;
        let subtract = if positive_digit {
            coeff_is_negative
        } else {
            !coeff_is_negative
        };
        terms.push((shift, subtract));
        if positive_digit {
            mag = (mag - 1) >> 1;
        } else {
            mag = (mag + 1) >> 1;
...
fn copy_signed_shifted_term_into_zero_scaffold(
    b: &mut super::B,
    src: &[super::QubitId],
    dst: &[super::QubitId],
    shift: usize,
    ...
    let copy_len = src.len().min(len);
    for i in 0..copy_len {
        b.cx(src[i], dst[shift + i]);
    }
    if !src.is_empty() {
        let sign = src[src.len() - 1];
        for i in copy_len..len {
            b.cx(sign, dst[shift + i]);
        }
    }
}

fn add_two_signed_coeffs_naf_free_positive_scaffold(
    b: &mut super::B,
    coeff0: i128,
    src0: &[super::QubitId],
    coeff1: i128,
    ...
    let mut terms: Vec<(usize, bool, usize)> = Vec::new();
    terms.extend(
        signed_coeff_naf_terms_for_scaffold(coeff0)
            .into_iter()
            .map(|(shift, subtract)| (shift, subtract, 0usize)),
    );
    terms.extend(
        signed_coeff_naf_terms_for_scaffold(coeff1)
            .into_iter()
            .map(|(shift, subtract)| (shift, subtract, 1usize)),
    );
    let free_idx = terms
        .iter()
        .enumerate()
        .filter(|(_, (shift, subtract, _))| !*subtract && *shift < dst.len())
        .max_by_key(|(_, (shift, _, src_idx))| {
            let src = if *src_idx == 0 { src0 } else { src1 };
            src.len().min(dst.len() - *shift)
        })
        .map(|(idx, _)| idx);
    if let Some(idx) = free_idx {
        let (shift, _, src_idx) = terms[idx];
        let src = if src_idx == 0 { src0 } else { src1 };
        copy_signed_shifted_term_into_zero_scaffold(b, src, dst, shift);
    }
    for (idx, (shift, subtract, src_idx)) in terms.into_iter().enumerate() {
        if Some(idx) == free_idx {
            continue;
        }
        let src = if src_idx == 0 { src0 } else { src1 };
        add_signed_shifted_term_naf_freepos_scaffold(b, src, dst, shift, subtract);
    }
}

fn arith_shift_right_inplace_scaffold(b: &mut super::B, v: &[super::QubitId], shift: usize) {
    let n = v.len();
...
pub(super) fn emit_round8_qtail_by_naf_freepos_scaffold(b: &mut super::B) {
    const W: usize = 16;
    const C0_WIDTH: usize = 17 * W;
    const C1_WIDTH: usize = 16 * W;
    const TMP_WIDTH: usize = C0_WIDTH + W;

    let (_, _, _, mtx) = jump_matrix_direct_lowword(W, W, 1, 1, 3);
    b.set_phase("round8_by_naf_freepos_scaffold_alloc");
    ...
    b.set_phase("round8_by_naf_freepos_scaffold_row0");
    add_two_signed_coeffs_naf_free_positive_scaffold(b, mtx.m00, &c0, mtx.m01, &c1, &y0);
    b.set_phase("round8_by_naf_freepos_scaffold_row1");
    add_two_signed_coeffs_naf_free_positive_scaffold(b, mtx.m10, &c0, mtx.m11, &c1, &y1);
    b.set_phase("round8_by_naf_freepos_scaffold_shift");
    arith_shift_right_inplace_scaffold(b, &y0, W);
    arith_shift_right_inplace_scaffold(b, &y1, W);
}
...
mod tests {
    ...
    fn fixed_by_coeff_channel_is_tagged_div_when_converged() {
        // Structural algebra for replacing Eea tagged-DIV with BY:
        ...
        // the BY analogue of the Eea y+x tagged DIV transform.
        let p = SECP256K1_P;
        let k = 550usize;
    ...
    fn jumpdivstep_budget_model_suggests_live_prototype() {
        ...
        // if this were already > Eea there would be no reason to prototype.
        const N: usize = 256;
        ...
        assert!(
            ...
            "BY lower bound no longer beats Eea enough to prototype"
        );
        assert!(
    ...
    fn add_signed_coeff_times_naf_for_cost(
        b: &mut super::super::B,
        coeff: i128,
        src: &[super::super::QubitId],
        dst: &[super::super::QubitId],
    ) {
        for (shift, subtract) in signed_coeff_naf_terms_for_by_budget(coeff) {
            add_signed_shifted_term_for_cost(b, src, dst, shift, subtract);
        }
    }

    fn signed_coeff_naf_terms_for_by_budget(coeff: i128) -> Vec<(usize, bool)> {
        let coeff_is_negative = coeff < 0;
        let mut mag = coeff.unsigned_abs();
        let mut shift = 0usize;
        let mut terms = Vec::new();
    ...
    fn copy_signed_shifted_term_into_zero_for_cost(
        b: &mut super::super::B,
        src: &[super::super::QubitId],
        dst: &[super::super::QubitId],
        shift: usize,
    ...
    fn add_two_signed_coeffs_naf_free_positive_for_cost(
        b: &mut super::super::B,
        coeff0: i128,
        ...
        let mut terms: Vec<(usize, bool, usize)> = Vec::new();
        terms.extend(
            signed_coeff_naf_terms_for_by_budget(coeff0)
                .into_iter()
                .map(|(shift, subtract)| (shift, subtract, 0usize)),
        );
        terms.extend(
            signed_coeff_naf_terms_for_by_budget(coeff1)
                .into_iter()
                .map(|(shift, subtract)| (shift, subtract, 1usize)),
        );
        let free_idx = terms
        ...
        if let Some(idx) = free_idx {
            let (shift, _, src_idx) = terms[idx];
            let src = if src_idx == 0 { src0 } else { src1 };
            copy_signed_shifted_term_into_zero_for_cost(b, src, dst, shift);
        }
        for (idx, (shift, subtract, src_idx)) in terms.into_iter().enumerate() {
            if Some(idx) == free_idx {
                continue;
    ...
    #[test]
    fn signed_naf_free_positive_row_former_matches_twos_complement() {
        // The low-gate BY row budget relies on a fresh zero row: one positive
        // NAF term may be copied in by CNOT fanout, then all remaining signed
        // terms are accumulated by reversible add/sub. Verify the actual gate
        // sequence against signed two's-complement arithmetic, including
        // negative source words and negative coefficients.
        const SRC0_WIDTH: usize = 12;
        const SRC1_WIDTH: usize = 11;
        const DST_WIDTH: usize = 20;
        let coeffs = [(57i128, -19i128), (-91, 37), (129, 85), (-255, -17)];
        let values = [
            (0i128, 0i128),
            (1, -1),
            (-17, 33),
        ...
        for &(coeff0, coeff1) in &coeffs {
            let mut b = super::super::B::new();
            let src0 = b.alloc_qubits(SRC0_WIDTH);
            let src1 = b.alloc_qubits(SRC1_WIDTH);
            let dst = b.alloc_qubits(DST_WIDTH);
            add_two_signed_coeffs_naf_free_positive_for_cost(
                &mut b, coeff0, &src0, coeff1, &src1, &dst,
            );
            let num_qubits = b.next_qubit as usize;
            let num_bits = b.next_bit as usize;
            let ops = b.ops;
            let modulus = 1i128 << DST_WIDTH;
            let mask = U512::from(1u64) << DST_WIDTH;
            let mask = mask - U512::from(1u64);

            for &(x0, x1) in &values {
                let mut hasher = sha3::Shake128::default();
                hasher.update(b"by-naf-free-positive-row-sim-v1");
                let mut xof = hasher.finalize_xof();
                let mut sim = crate::sim::Simulator::new(num_qubits, num_bits, &mut xof);
                set_slice_u512_by(&mut sim, &src0, twos_u512_for_delta(x0 as i64, SRC0_WIDTH));
                set_slice_u512_by(&mut sim, &src1, twos_u512_for_delta(x1 as i64, SRC1_WIDTH));
                sim.apply(&ops);

                let expected = (coeff0 * x0 + coeff1 * x1).rem_euclid(modulus) as u128;
                assert_eq!(
                    get_slice_u512_by(&sim, &dst) & mask,
                    U512::from(expected),
                    "row mismatch coeffs=({coeff0},{coeff1}) values=({x0},{x1})"
                );
                assert_eq!(
                    get_slice_u512_by(&sim, &src0),
                    twos_u512_for_delta(x0 as i64, SRC0_WIDTH),
                    "src0 changed"
                );
                assert_eq!(
                    get_slice_u512_by(&sim, &src1),
                    twos_u512_for_delta(x1 as i64, SRC1_WIDTH),
                    "src1 changed"
                );
    ...
    fn tail_carry_fresh_update_naf_cost_for_by_budget(mtx: TransitionMatrix) -> usize {
        const C0_WIDTH: usize = 17 * 16;
        const C1_WIDTH: usize = 16 * 16;
        const TMP_WIDTH: usize = C0_WIDTH + 16;
        let mut b = super::super::B::new();
        ...
        let y0 = b.alloc_qubits(TMP_WIDTH);
        let y1 = b.alloc_qubits(TMP_WIDTH);
        add_signed_coeff_times_naf_for_cost(&mut b, mtx.m00, &c0, &y0);
        add_signed_coeff_times_naf_for_cost(&mut b, mtx.m01, &c1, &y0);
        add_signed_coeff_times_naf_for_cost(&mut b, mtx.m10, &c0, &y1);
        add_signed_coeff_times_naf_for_cost(&mut b, mtx.m11, &c1, &y1);
        arith_shift_right_inplace_for_cost(&mut b, &y0, 16);
        arith_shift_right_inplace_for_cost(&mut b, &y1, 16);
        count_ccx(&b.ops)
    }

    fn tail_carry_fresh_update_naf_freepos_cost_for_by_budget(mtx: TransitionMatrix) -> usize {
        const C0_WIDTH: usize = 17 * 16;
        const C1_WIDTH: usize = 16 * 16;
        const TMP_WIDTH: usize = C0_WIDTH + 16;
        let mut b = super::super::B::new();
        ...
        let y0 = b.alloc_qubits(TMP_WIDTH);
        let y1 = b.alloc_qubits(TMP_WIDTH);
        add_two_signed_coeffs_naf_free_positive_for_cost(&mut b, mtx.m00, &c0, mtx.m01, &c1, &y0);
        add_two_signed_coeffs_naf_free_positive_for_cost(&mut b, mtx.m10, &c0, mtx.m11, &c1, &y1);
        arith_shift_right_inplace_for_cost(&mut b, &y0, 16);
        arith_shift_right_inplace_for_cost(&mut b, &y1, 16);
        count_ccx(&b.ops)
    }
    ...
    #[test]
    fn first16_tail_carry_naf_update_closes_lowgate_arithmetic_gap() {
        // The binary fresh-update checkpoint above is too pessimistic relative to
        // the CSD lower bound: the BY window matrices are signed and sparse under
        // non-adjacent form.  This measures the same forward-only fresh rows with
        // signed NAF terms.  It still is not a complete PA circuit because old
        // carry rows, pattern generation, and reversible parser cleanup are not
        // charged here, but it determines whether the arithmetic row is worth
        // implementing instead of discarding the whole first16/tail path.
        use std::collections::HashMap;
        const W: usize = 16;
        const PREFIX_WINDOWS: usize = 16;
        const WINDOWS: usize = 35;
        const SAMPLES: usize = 64;
        let mut sampler = Sampler::new(b"by-tail-carry-naf-circuit-v1", SECP256K1_P);
        let mut cache: HashMap<(i128, i128, i128, i128), usize> = HashMap::new();
        let mut costs = Vec::with_capacity(SAMPLES);
        for _ in 0..SAMPLES {
            let x = sampler.next();
            ...
            for win in 0..WINDOWS {
                ...
                if win >= PREFIX_WINDOWS {
                    ...
                    let c = *cache
                        .entry(key)
                        .or_insert_with(|| tail_carry_fresh_update_naf_cost_for_by_budget(m));
                    cost += c;
                }
                for _ in 0..W {
                    divstep_sint_state(&mut delta, &mut f, &mut g);
        ...
        let min_cached_window = cache.values().copied().min().unwrap_or(0);
        let max_cached_window = cache.values().copied().max().unwrap_or(0);
        eprintln!("BY tail carry NAF-update circuit: unique_matrices={}, min_window={min_cached_window}, max_window={max_cached_window}, mean={mean:.1}, p90={p90}, p99={p99}, max={max}, p99_gap={p99_gap}, max_gap={max_gap}", cache.len());
        println!("METRIC by_tail_carry_naf_unique_matrices={}", cache.len());
        println!("METRIC by_tail_carry_naf_window_min_ccx={min_cached_window}");
        println!("METRIC by_tail_carry_naf_window_max_ccx={max_cached_window}");
        println!("METRIC by_tail_carry_naf_mean_ccx={mean:.3}");
        println!("METRIC by_tail_carry_naf_p90_ccx={p90}");
        println!("METRIC by_tail_carry_naf_p99_ccx={p99}");
        println!("METRIC by_tail_carry_naf_max_ccx={max}");
        println!("METRIC by_tail_carry_naf_p99_gap_to_remaining_ccx={p99_gap}");
        println!("METRIC by_tail_carry_naf_max_gap_to_remaining_ccx={max_gap}");
        assert!(
            p99_gap <= 2_500,
            "NAF row former drifted away from the first16/tail low-gate arithmetic gap"
        );
    }

    #[test]
    fn first16_tail_carry_naf_free_positive_update_closes_lowgate_arithmetic_gap() {
        // Fresh rows start at zero, so one positive signed-NAF source term per row
        // can be initialized by CNOT fanout instead of an adder. This is the
        // circuit version of the free-first-term assumption in the lower bound.
        use std::collections::HashMap;
        const W: usize = 16;
        const PREFIX_WINDOWS: usize = 16;
        const WINDOWS: usize = 35;
        const SAMPLES: usize = 64;
        let mut sampler = Sampler::new(b"by-tail-carry-naf-freepos-circuit-v1", SECP256K1_P);
        let mut cache: HashMap<(i128, i128, i128, i128), usize> = HashMap::new();
        let mut costs = Vec::with_capacity(SAMPLES);
        for _ in 0..SAMPLES {
            let x = sampler.next();
            ...
            for win in 0..WINDOWS {
                ...
                if win >= PREFIX_WINDOWS {
                    let key = (m.m00, m.m01, m.m10, m.m11);
                    let c = *cache.entry(key).or_insert_with(|| {
                        tail_carry_fresh_update_naf_freepos_cost_for_by_budget(m)
                    });
                    cost += c;
                }
                for _ in 0..W {
        ...
        let min_cached_window = cache.values().copied().min().unwrap_or(0);
        let max_cached_window = cache.values().copied().max().unwrap_or(0);
        eprintln!("BY tail carry NAF free-positive update: unique_matrices={}, min_window={min_cached_window}, max_window={max_cached_window}, mean={mean:.1}, p90={p90}, p99={p99}, max={max}, p99_gap={p99_gap}, max_gap={max_gap}", cache.len());
        println!(
            "METRIC by_tail_carry_naf_freepos_unique_matrices={}",
            cache.len()
        );
        println!("METRIC by_tail_carry_naf_freepos_window_min_ccx={min_cached_window}");
        println!("METRIC by_tail_carry_naf_freepos_window_max_ccx={max_cached_window}");
        println!("METRIC by_tail_carry_naf_freepos_mean_ccx={mean:.3}");
        println!("METRIC by_tail_carry_naf_freepos_p90_ccx={p90}");
        println!("METRIC by_tail_carry_naf_freepos_p99_ccx={p99}");
        println!("METRIC by_tail_carry_naf_freepos_max_ccx={max}");
        println!("METRIC by_tail_carry_naf_freepos_p99_gap_to_remaining_ccx={p99_gap}");
        println!("METRIC by_tail_carry_naf_freepos_max_gap_to_remaining_ccx={max_gap}");
        assert!(p99_gap < 0, "free-positive NAF row former does not close the p99 first16/tail low-gate arithmetic gap");
    }
    ...
    fn subtract_signed_q_times_fast_modulo_c_for_cost(
        b: &mut super::super::B,
    ...
    fn emit_fixed_matrix_old_cleanup_with_mq_for_test(
        ...
        subtract_signed_q_times_fast_modulo_c_for_cost(b, q0, x0);
        subtract_signed_q_times_fast_modulo_c_for_cost(b, q1, x1);
    ...
    fn by_fixed_window_three_million_budget_still_misses_with_current_arithmetic() {
        ...
        // replays and the deleted-Eea scaffold, the two denominator windows
    ...
    fn streamed_mask_controlled_qoffset_fits_scratch_and_hits_lowqubit_target() {
        ...
        let projected_ec_add = div560 as usize + scaffold_after_div + branch_decode_margin;
        let scratch_with_history = 481usize + 26 + 3;
        eprintln!(
            "streamed-mask controlled qoffset: ccx={ccx}, peak={peak}q, div560≈{div560:.0}, projected≈{projected_ec_add}, scratch_with_history≈{scratch_with_history}q"
        );
        ...
        println!("METRIC streamed_mask_qoffset_projected_ec_add={projected_ec_add}");
    ...
    fn partial_prefix_qoffset_two_denominator_ledger_blocks_naive_promotion() {
        ...
        // state_a tagged-DIV and state_b product-clean replay, the partial-prefix
        ...
        let current_two_eea = 3_190_000isize;
        let deleted_state_a_muls = 149_889isize + 150_145isize;
        let state_a_scale_loop = 407isize * 255;
        let state_b_scale_loop = 404isize * 255;
        let state_b_product_mul = 150_145isize;
        let two_replay_scaffold = current_total
            - current_two_eea
            - deleted_state_a_muls
            - state_a_scale_loop
            - state_b_scale_loop
            - state_b_product_mul;
        let two_den_total =
    ...
    fn partial_prefix_one_div_is_too_expensive_for_strategy_e_escape() {
        ...
        assert!(
            hypothetical_gap > 0,
            "partial-prefix DIV plus schoolbook product would make Strategy E viable; revisit"
        );
    }
    ...
    fn strategy_e_three_million_still_needs_unavailable_div_selector_budget() {
        ...
        assert!(
            partial_prefix_gap > 0,
            "partial-prefix DIV is enough for Strategy E under 3M; revisit"
        );
    }
    ...
    #[test]
    fn strategy_e_product_denominator_controls_do_not_share_state_a_branch_stream() {
        ...
        assert!(
            ...
            "state_a/product odd controls show exploitable correlation; investigate sharing"
        );
        assert!(
            case_mi_millibits < 10.0,
            "state_a/product branch cases show exploitable correlation; investigate sharing"
        );
    }
    ...
    fn partial_mask_controlled_qoffset_linear_tradeoff_just_misses_600q_target() {
        ...
        println!("METRIC partial_mask_qoffset_projected_ec_add={projected}");
    ...
    fn clean_two_replay_by_budget_requires_replay_or_phase_breakthrough() {
        ...
        let deleted_state_a_muls = 149_889.0 + 150_145.0;
        let two_replay_scaffold =
            non_inv_scaffold - deleted_state_a_muls - 407.0 * 255.0 - 404.0 * 255.0 - 150_145.0;
    ...
    #[test]
    fn low_scratch_scaled_by_budget_still_beats_27m_after_state_a_mul_deletion() {
        // Important refinement: a true tagged DIV does not merely replace the
        // two Eea bodies. It also deletes state_a's two schoolbook
        ...
        let current_two_eea = 3_190_000.0;
        let non_inv_scaffold = current_total - current_two_eea;
        let deleted_state_a_muls = 149_889.0 + 150_145.0;
        let scaffold_after_div = non_inv_scaffold - deleted_state_a_muls;
        ...
        let two_replay_scaffold = current_total
            - current_two_eea
            - deleted_state_a_muls
            - 407.0 * 255.0 // state_a scale loop
            - 404.0 * 255.0 // state_b double/scale loop
            - 150_145.0; // state_b schoolbook product add
        ...
        eprintln!(
            "BY DIV with state_a mul deletion: scaffold_after_div≈{scaffold_after_div:.0}, fast_projected≈{fast_projected:.0}, low_scratch_vented_projected≈{low_scratch_projected:.0}, two_fast_replays≈{two_fast_replays:.0}"
        );
        assert!(
    ...
    #[test]
    fn scaled_by_div_ec_add_budget_has_sota_margin_if_history_workspace_solved() {
        // The structural point of the scaled controlled microstep is that it
        // replaces both Eea invocations by one in-place tagged DIV. This is
        ...
        let current_total = 4_132_750.0;
        let current_two_eea = 3_190_000.0; // measured ~1.60M + ~1.59M from point-add decomposition.
        let non_inv_scaffold = current_total - current_two_eea;
    ...
    fn ripple_adder_add_fast_negphase_for_test(
        b: &mut super::super::B,
        a: &[super::super::QubitId],
        acc: &[super::super::QubitId],
    ...
    fn ripple_adder_sub_fast_negphase_for_test(
        b: &mut super::super::B,
        a: &[super::super::QubitId],
        acc: &[super::super::QubitId],
        c_in: super::super::QubitId,
    ...
    fn add_nbit_qq_fast_negphase_for_test(
        ...
        ripple_adder_add_fast_negphase_for_test(b, a, acc, c_in);
        b.free(c_in);
    }
    ...
    fn sub_nbit_qq_fast_negphase_for_test(
        ...
        let c_in = b.alloc_qubit();
        ripple_adder_sub_fast_negphase_for_test(b, a, acc, c_in);
        b.free(c_in);
    }
    ...
    fn emit_pattern_delta_decode_window_for_test(
        ...
        for i in 0..pattern.len() {
            ...
            // Use exact RippleAdder add here. The decoder is run forward, then its
    ...
    fn inverse_scaled_by_560_cleans_lam_and_writes_product() {
        // This is the missing state_b cleanup schedule. If forward scaled BY maps
        // (0, q*x) -> (sign*q, 0), then the inverse map sends (sign*q, 0) ->
        // (0, q*x). Thus state_b can clean lam and write Ry+Qy without a
        // Eea-style inversion or a separate q*x multiplication.
        let p = SECP256K1_P;
    ...
    fn emit_centered_poststep_parity_clear_unified_for_test(
        b: &mut super::super::B,
        r_out: &[super::super::QubitId],
        s_out: &[super::super::QubitId],
        odd_ctrl: super::super::QubitId,
        ...
        // For all BY cases, including A by sign symmetry, the pre-halve
        // centered parity is !centered(2*s_out - odd*r_out).
        let n = s_out.len();
        let tmp = b.alloc_qubits(n);
        for i in 0..n - 1 {
            b.cx(s_out[i], tmp[i + 1]);
        }
        emit_signed_controlled_sub_for_test(b, &tmp, r_out, odd_ctrl);
        let centered = b.alloc_qubit();
        let bias = p >> 1usize;
        super::super::add_nbit_const_fast(b, &tmp, bias);
        let p_reg = super::super::load_const(b, n, p);
        super::super::cmp_lt_into_fast(b, &tmp, &p_reg, centered);
        b.x(centered);
        b.cx(centered, parity_hist);
        b.x(centered);
        super::super::cmp_lt_into_fast(b, &tmp, &p_reg, centered);
        super::super::unload_const(b, &p_reg, p);
        super::super::sub_nbit_const_fast(b, &tmp, bias);
        emit_signed_controlled_add_for_test(b, &tmp, r_out, odd_ctrl);
        for i in (0..n - 1).rev() {
            b.cx(s_out[i], tmp[i + 1]);
        }
        b.free(centered);
    ...
    fn emit_centered_poststep_parity_clear_constcmp_for_test(
        b: &mut super::super::B,
        r_out: &[super::super::QubitId],
        s_out: &[super::super::QubitId],
        odd_ctrl: super::super::QubitId,
        ...
        // Same identity as the unified clear:
        // parity = !centered(2*s_out - odd*r_out).
        // Avoid the explicit p register in cmp_lt_into_fast by comparing tmp
        // against p-1 as a compile-time constant.
        let n = s_out.len();
        let tmp = b.alloc_qubits(n);
        for i in 0..n - 1 {
            b.cx(s_out[i], tmp[i + 1]);
        ...
        emit_signed_controlled_sub_for_test(b, &tmp, r_out, odd_ctrl);
        let outside = b.alloc_qubit();
        let bias = p >> 1usize;
        super::super::add_nbit_const(b, &tmp, bias);
        b.cx(tmp[n - 1], parity_hist);
        super::super::cmp_gt_const_n1(b, &tmp, p.wrapping_sub(U256::from(1u64)), outside);
        b.cx(outside, parity_hist);
        super::super::cmp_gt_const_n1(b, &tmp, p.wrapping_sub(U256::from(1u64)), outside);
        super::super::sub_nbit_const(b, &tmp, bias);
        emit_signed_controlled_add_for_test(b, &tmp, r_out, odd_ctrl);
        for i in (0..n - 1).rev() {
            b.cx(s_out[i], tmp[i + 1]);
        }
    ...
    #[test]
    fn centered_unified_poststep_parity_clear_matches_all_cases() {
        const WIDE: usize = 260;
        let p = SECP256K1_P;
        let p512 = u256_to_u512_for_by_tests(p);
        let mut b = super::super::B::new();
        ...
        emit_centered_poststep_parity_clear_unified_for_test(&mut b, &r, &s, odd, parity, p);
        let ccx = count_ccx(&b.ops);
        let peak = b.peak_qubits;
        let num_qubits = b.next_qubit as usize;
        let num_bits = b.next_bit as usize;
        let ops = b.ops;
        let cases = [(false, false, "C"), (true, false, "B"), (true, true, "A")];
        let mut sx = Sampler::new(b"by-centered-clear1-r-v1", p);
        let mut sy = Sampler::new(b"by-centered-clear1-s-v1", p);
        for &(odd_v, a_v, name) in &cases {
            for _ in 0..16 {
                ...
                let mut hasher = sha3::Shake128::default();
                hasher.update(b"by-centered-clear1-sim-v1");
                let mut xof = hasher.finalize_xof();
                let mut sim = crate::sim::Simulator::new(num_qubits, num_bits, &mut xof);
                if odd_v {
                    *sim.qubit_mut(odd) |= 1;
                ...
                assert_eq!(sim.qubit(parity) & 1, 0, "parity not cleared {name}");
                assert_eq!(sim.global_phase() & 1, 0, "phase garbage {name}");
            }
        }
        eprintln!("BY centered unified poststep parity clear: ccx={ccx}, peak={peak}q");
        assert!(
            ccx < 4_000,
            "unified poststep parity clear is too expensive per step"
        );
        assert!(
            peak < 1_600,
            "unified poststep parity clear peak is too high for local primitive"
        );
    }

    #[test]
    fn centered_constcmp_poststep_parity_clear_is_correct_but_not_a_width_fix() {
        const WIDE: usize = 260;
        let p = SECP256K1_P;
        let p512 = u256_to_u512_for_by_tests(p);
        let mut b = super::super::B::new();
        ...
        emit_centered_poststep_parity_clear_constcmp_for_test(&mut b, &r, &s, odd, parity, p);
        let ccx = count_ccx(&b.ops);
        let peak = b.peak_qubits;
        let num_qubits = b.next_qubit as usize;
        let num_bits = b.next_bit as usize;
        let ops = b.ops;
        let cases = [(false, false, "C"), (true, false, "B"), (true, true, "A")];
        let mut sx = Sampler::new(b"by-centered-constcmp-clear1-r-v1", p);
        let mut sy = Sampler::new(b"by-centered-constcmp-clear1-s-v1", p);
        for &(odd_v, a_v, name) in &cases {
            for _ in 0..16 {
                let rv = sw_centered_from_u256_for_test(sx.next(), p);
                let sv = sw_centered_from_u256_for_test(sy.next(), p);
                ...
                let mut hasher = sha3::Shake128::default();
                hasher.update(b"by-centered-constcmp-clear1-sim-v1");
                let mut xof = hasher.finalize_xof();
                let mut sim = crate::sim::Simulator::new(num_qubits, num_bits, &mut xof);
                if odd_v {
                    *sim.qubit_mut(odd) |= 1;
        ...
        eprintln!("BY centered constcmp poststep parity clear: ccx={ccx}, peak={peak}q");
        assert!(
            ccx > 5_000,
            "const comparator unexpectedly became gate-competitive"
        );
        assert!(
            peak >= 1_300,
            "const comparator unexpectedly solved the selector-width gap"
        );
    }

    #[test]
    fn centered_signed_560_immediate_parity_clear_scaffold_is_clean() {
        const WIDE: usize = 260;
        const STEPS: usize = 560;
        let p = SECP256K1_P;
        let p512 = u256_to_u512_for_by_tests(p);
        let mut sx = Sampler::new(b"by-centered-clear560-x-v1", p);
        let mut sy = Sampler::new(b"by-centered-clear560-y-v1", p);
        ...
        for i in 0..STEPS {
            emit_scaled_by_centered_signed_microstep_live_parity_for_test(
                &mut b, &r, &s, odd[i], a_ctrl[i], parity, p,
            );
            emit_centered_poststep_parity_clear_unified_for_test(&mut b, &r, &s, odd[i], parity, p);
        }
        let ccx = count_ccx(&b.ops);
        let peak = b.peak_qubits;
        let num_qubits = b.next_qubit as usize;
        ...
        let ops = b.ops;
        let mut hasher = sha3::Shake128::default();
        hasher.update(b"by-centered-clear560-sim-v1");
        let mut xof = hasher.finalize_xof();
        let mut sim = crate::sim::Simulator::new(num_qubits, num_bits, &mut xof);
        for (i, &(odd_v, a_v)) in controls.iter().enumerate() {
            if odd_v {
        ...
        assert_eq!(
            get_slice_u512_by(&sim, &r),
            sw_twos_for_width_for_test(exp_r, WIDE),
            "centered clear560 r mismatch"
        );
        assert_eq!(
            get_slice_u512_by(&sim, &s),
            sw_twos_for_width_for_test(exp_s, WIDE),
            "centered clear560 s mismatch"
        );
        assert_eq!(
        ...
        assert_eq!(
            quotient,
            mulm(y, fermat_modinv(x, p), p),
            "centered immediate-clear quotient mismatch"
        );
        assert_eq!(sim.qubit(parity) & 1, 0, "immediate-clear parity bit dirty");
        eprintln!(
            "BY centered signed 560 immediate parity-clear scaffold: ccx={ccx}, peak={peak}q, phase={}",
            sim.global_phase() & 1
        );
        assert_eq!(
            sim.global_phase() & 1,
            0,
            "immediate parity-clear phase garbage"
        );
        assert!(
            ccx < 2_100_000,
            "immediate parity-clear replay exceeds one-DIV Google gate band"
        );
        assert!(
            peak < 2_500,
            "immediate parity-clear scaffold too wide before selector compression"
        );
    }
    ...
    #[test]
    fn centered_signed_fully_fast_full_clean_after_unhalve_fix_is_phase_clean() {
        const WIDE: usize = 260;
        const STEPS: usize = 560;
        let p = SECP256K1_P;
        let mut b = super::super::B::new();
        ...
        for i in 0..STEPS {
            emit_scaled_by_centered_signed_microstep_live_parity_variant_for_test(
                &mut b, &r, &s, odd[i], a_ctrl[i], parity[i], p, false, false,
            );
        }
        for i in (0..STEPS).rev() {
            emit_scaled_by_centered_signed_microstep_inverse_live_parity_variant_for_test(
        ...
        let two_clean_div_projection = NON_DIV_SCAFFOLD + 2 * ccx;
        eprintln!(
            "BY centered fully-fast full clean budget: ccx={ccx}, peak={peak}q, two_clean_div_projection={two_clean_div_projection}"
        );
        assert!(
            ccx < 1_800_000,
            "fully-fast centered replay lost the clean-roundtrip target band"
        );
        assert!(
            two_clean_div_projection > GOOGLE_LOW_QUBIT_TOFFOLI,
            "two fully-fast centered BY DIVs now fit the 2.7M screen; revisit PA promotion"
        );
        let num_qubits = b.next_qubit as usize;
        let num_bits = b.next_bit as usize;
        let ops = b.ops;
        let mut sx = Sampler::new(b"by-centered-fastclean-phase-x-v1", p);
        let mut sy = Sampler::new(b"by-centered-fastclean-phase-y-v1", p);
        let mut saw = [false; 2];
        for sample in 0..24 {
            let x = sx.next();
            let y = sy.next();
            ...
            let mut hasher = sha3::Shake128::default();
            hasher.update(b"by-centered-fastclean-phase-sim-v1");
            hasher.update(&(sample as u64).to_le_bytes());
            let mut xof = hasher.finalize_xof();
            let mut sim = crate::sim::Simulator::new(num_qubits, num_bits, &mut xof);
            for (i, &(odd_v, a_v)) in controls.iter().enumerate() {
        ...
        eprintln!(
            "BY centered fully-fast full clean phases after unhalve fix: saw0={}, saw1={}",
            saw[0], saw[1]
        );
        assert!(
            saw[0] && !saw[1],
            "fully-fast centered clean replay still has data-dependent phase"
        );
    }
    ...
    #[test]
    fn negating_ripple_adder_carry_measurements_does_not_fix_centered_control_phase() {
        const WIDE: usize = 260;
        const STEPS: usize = 560;
        let p = SECP256K1_P;
        let mut b = super::super::B::new();
        ...
        assert!(
            saw[0] && saw[1],
            "neg_if on RippleAdder carry measurements unexpectedly fixed the cleaned-control phase"
        );
    }
    ...
    fn by_tagged_div_stored_matrix_upper_bound_model() {
        ...
        assert!(
            ...
            "stored-matrix BY arithmetic no longer cheaper than Eea"
        );
        assert!(
    ...
    fn hybrid_jump_denominator_with_microstep_tag_channel_still_too_costly() {
        ...
        assert!(
            ...
            "hybrid unexpectedly beats Eea; revisit implementation path"
        );
    }
    ...
    fn jumpdivstep_full_state_budget_model() {
        ...
        // RippleAdder carry strip. This is the first budget that includes both

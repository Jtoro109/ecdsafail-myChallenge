//! Classical replay of the current Eea iteration to test whether `m_i`
//! is recoverable from a small live-state window `(u[0], v_w[0], s[0],
//! iter_idx, f)` at iteration start.
//!
//! Purpose: feasibility check for §7/§9 of `eea_1200q_feasibility.md`.
//! If `m_i` can be recomputed from a constant-size fingerprint of the live
//! Eea state at each iteration, then `m_hist` (407 qubits) is not
//! persistent state and Eea fits in 1200q (together with the other
//! compressions analyzed).
//!
//! This is analysis-only; does not change the quantum circuit. Run via:
//!     EEA_REPLAY=1 cargo test --release classical_replay
//! or via the top-level harness by calling
//! `eea_classical_replay::run_feasibility_test()`.
...
/// One Eea iteration, classical replay of the quantum circuit in
/// `mod.rs::eea_iteration`. Returns `m_i` for this iteration and
...
pub fn eea_iter_classical(
    ...
    *f ^= m_i;
    ...
    if add_f_step4 == 1 {
        // v_w -= u (mod 2^n for the register width, but in reality eea
        ...
        *v_w = v_w.wrapping_sub(*u);
        ...
        // but the Eea invariant keeps s < p always — check).
        let sum = s.wrapping_add(*r);
        *s = if sum < *s || sum >= p {
            // carry or overflow beyond p? Eea invariant says s+r ≤ p,
...
/// Classical driver: run `iters` iterations of Eea on input `v_in`,
...
pub fn eea_run(
    ...
    for _i in 0..iters {
        ...
        let m = eea_iter_classical(&mut u, &mut v_w, &mut r, &mut s, &mut f, p);
        m_hist.push(m);
    }
...
pub struct FingerprintConflictSummary {
    pub n_inputs: usize,
    pub iters_per_input: usize,
    pub total_samples: usize,
    pub f1_conflicts: usize,
    pub f2_conflicts: usize,
    pub f3_conflicts: usize,
    pub f4_conflicts: usize,
    pub fmin_conflicts: usize,
}

impl FingerprintConflictSummary {
    pub fn f4_is_deterministic(self) -> bool {
        self.f4_conflicts == 0
    }
}
...
pub fn fingerprint_conflict_summary(
    n_inputs: usize,
    iters_per_input: usize,
) -> FingerprintConflictSummary {
    let p = SECP256K1_P;
    ...
    for seed in 0..n_inputs {
        ...
        let (m_hist, snaps) = eea_run(v_in, p, iters_per_input);
        for i in 0..iters_per_input {
            ...
            match f1.get(&k1) {
                ...
                Some(&v) if v != m_i => f1_conflicts += 1,
                _ => {}
            }
            ...
            match f2.get(&k2) {
                ...
                Some(&v) if v != m_i => f2_conflicts += 1,
                _ => {}
            }
            ...
            match f3.get(&k3) {
                ...
                Some(&v) if v != m_i => f3_conflicts += 1,
                _ => {}
            }
            ...
            match f4.get(&k4) {
                ...
                Some(&v) if v != m_i => f4_conflicts += 1,
                _ => {}
            }
            ...
            match fmin.get(&kmin) {
                ...
                Some(&v) if v != m_i => fmin_conflicts += 1,
                _ => {}
            }
...
pub fn run_feasibility_test(n_inputs: usize, iters_per_input: usize) -> bool {
    let summary = fingerprint_conflict_summary(n_inputs, iters_per_input);
    let f1_conflict_examples: Vec<(u8, u8)> = Vec::new();
    let f4_conflict_examples: Vec<(u8, u8)> = Vec::new();

    println!(
        "=== Eea m_i-from-fingerprint feasibility test ===
inputs={} iters/input={} total_samples={}",
        summary.n_inputs, summary.iters_per_input, summary.total_samples
    );
    println!(
        "F1 (f,u0,v0): {} conflicts (examples: {:?})",
        summary.f1_conflicts, f1_conflict_examples
    );
    println!("F2 (f,u0,v0,s0): {} conflicts", summary.f2_conflicts);
    println!("F3 (f,u0,v0,r0,s0): {} conflicts", summary.f3_conflicts);
    println!(
        "F4 (f,u0,v0,u>v_w,s0): {} conflicts (examples: {:?})",
        summary.f4_conflicts, f4_conflict_examples
    );
    println!(
        "F_min (f,u0,v0,u>v_w): {} conflicts",
        summary.fmin_conflicts
    );
    ...
    let f4_is_deterministic = summary.f4_is_deterministic();
...
Qubit-budget implication: Eea persistent state drops by 407 qubits
(from 1432 → 1025), making single-inversion Eea + aggressive
...
Implication: Eea's m_hist is NOT compressible via simple HRSL-style
recomputation. Eea at 1200q requires either
  (a) Kim unconditional execution (+28% Toffoli), or
  (b) Bennett pebbling (multiplicative Toffoli overhead), or
  (c) replacing Eea with a non-Eea inverter.",
            summary.f4_conflicts
        );
    }
...
mod tests {
    ...
    fn classical_replay_matches_self() {
        ...
        let (m_hist, snaps) = eea_run(v_in, SECP256K1_P, 512);
        ...
        assert!(any_terminated, "Eea never terminated in 512 iters");
    }
    ...
    fn eea_iter_with_af(
        ...
        // Replay EXACTLY as eea_iter_classical but also return a_f.
        let is_zero = if *v_w == U256::ZERO { 1u8 } else { 0 };
        let mut m_i: u8 = 0;
        if *f == 1 && is_zero == 1 {
            m_i ^= 1;
    ...
    fn check_iter_end_plus_af_fingerprint() {
        ...
        for seed in 0..500u64 {
            ...
            let mut f: u8 = 1;
            for _i in 0..512 {
                let (m_i, a_f) = eea_iter_with_af(&mut u, &mut v_w, &mut r, &mut s, &mut f, p);
                let u0 = (u.as_limbs()[0] & 1) as u8;
                let v0 = (v_w.as_limbs()[0] & 1) as u8;
                let gt = if u > v_w { 1u8 } else { 0 };
                let s0 = (s.as_limbs()[0] & 1) as u8;
    ...
    fn check_iter_end_fingerprint() {
        ...
        for seed in 0..200u64 {
            ...
            let mut f: u8 = 1;
            for _i in 0..512 {
                let m_i = eea_iter_classical(&mut u, &mut v_w, &mut r, &mut s, &mut f, p);
    ...
    fn persistent_orientation_cswap_savings_have_selector_caveat() {
        // First-principles check of the "batch consecutive Eea swaps" idea.
        // Current Eea emits pre- and post-cswaps whenever a_i=1.  If we
        ...
        for seed in 0..samples as u64 {
            let v_in = random_element(seed + 1);
            let (_m_hist, snaps) = eea_run(v_in, p, iters);
            let mut prev = 0u8;
            for i in 0..iters {
        ...
        eprintln!(
            "Eea persistent orientation upper bound: a_frac={a_frac:.6}, event_frac={event_frac:.6}, cswap_event_ratio={ratio:.6}"
        );
        println!("METRIC eea_persistent_orientation_a_frac={a_frac:.6}");
        println!("METRIC eea_persistent_orientation_event_frac={event_frac:.6}");
        println!("METRIC eea_persistent_orientation_cswap_ratio={ratio:.6}");
        assert!(ratio < 0.75);
    ...
    #[test]
    fn transition_oriented_eea_matches_canonical_classically() {
        // Classical skeleton for a possible Eea persistent-orientation
        ...
        for seed in 0..samples as u64 {
            ...
            for _ in 0..iters {
                ...
                let m_ref = eea_iter_classical(&mut cu, &mut cv, &mut cr, &mut cs, &mut cf, p);
                assert_eq!(m_i, m_ref);
                of = f_after_zero;
        ...
        eprintln!(
            "Transition-oriented Eea classical swap-word ratio: {total_swaps}/{canonical_swaps} = {ratio:.6}"
        );
        println!("METRIC eea_transition_orientation_swap_ratio={ratio:.6}");
        assert!(ratio < 0.35);
    }
    ...
    fn transition_orientation_selector_floor_kills_swap_saving() {
        ...
        // local win in the current Eea body.
        const N: f64 = 256.0;
        let samples = 2048usize;
        ...
        eprintln!(
            "Transition-oriented Eea selector floor: save404={cswap_saving_404:.1}, save401={cswap_saving_401:.1}, selector_floor={selector_floor:.1}, projected_pointadd_loss={projected_pointadd_loss:.0}"
        );
        println!("METRIC eea_transition_cswap_saving_ccx_per_iter_404={cswap_saving_404:.3}");
        println!("METRIC eea_transition_cswap_saving_ccx_per_iter_401={cswap_saving_401:.3}");
        println!("METRIC eea_transition_selector_floor_ccx_per_iter={selector_floor:.3}");
        println!(
            "METRIC eea_transition_projected_pointadd_loss_ccx={projected_pointadd_loss:.0}"
        );
    ...
    fn verify_minimal_formula() {
        ...
        for seed in 0..500u64 {
            let v_in = random_element(seed + 1);
            let (m_hist, snaps) = eea_run(v_in, p, 512);
            for i in 0..512 {

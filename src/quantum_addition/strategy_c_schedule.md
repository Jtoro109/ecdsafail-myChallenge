...
| schoolbook_addsub fwd (fast RippleAdder)       | 1283           |
| mod_add_qq_fast                            | ~n + n = 512 (?) — need to verify |
| Eea forward body peak                   | ~2204 (our baseline incl tx,ty,lam) |
...
- `w_reg` (256): holds dx³, passed as Eea `v_in`. Alive from
  pre-Eea through post-Eea (mostly, until uncomputed). This is
  an extra register not present in current design (current design uses
  `tx` directly as `v_in`).
- `r_low` ⊆ Eea state (256): Eea output. Alive after forward,
  until backward consumes it.
- Eea internal state during fwd/bwd: u, v_w, s, f_flag = 1025q
...
- some RippleAdder carries + ancillas (~n = 256)
...
### Phase 2: Eea on w
  ...
  -----|--------------------------------------------|-----------------------------------|-----
   2.1 | Eea forward on w_reg                   | tx, ty, dx2_reg, w_reg + kal_state | ~1024+1432+fwd transient
   ...
   2.5 | Eea backward on w_reg                   | tx, ty, dx2_reg, w_reg + kal_state | ~1024+1432+bwd transient

**Peak during Eea forward**: 1024 (pre-Eea) + 1432 (full kal
state incl m_hist) + ~260 (step 4 transient) = ~2716.
Same as current! Because current circuit has similar structure (tx+ty+lam=768+lam_256=1024 persistent inside body + kal state). Actually current has lam NOT dx2 during Eea. Either way, peak = ~2716.
...
**Mitigation**: use `KAL_FREE_S=1` style tricks to free MORE Eea
state during body. `m_hist` is 407q. Is there a way to free it during
body? `m_hist` is needed during backward Eea. So no.
...
Can we re-derive dx² from live state after Eea backward without
...
The fundamental issue is: **mul transients + 1432 Eea state +
...
The fundamental tension: **the peak inside the Eea body is
bounded by (Eea state + tx + ty + other-alive-intermediates + mul
transient)**. Currently our 2-Eea scaffold has (r+m_hist + tx + ty +
lam + mul transient) = 256+407+256+256+256+260 ≈ 1700. Peak ~2716 includes
step-4 transient during backward Eea.
...
Eea backward) = 2715. Right at current peak.
...
Phase 2 (Eea on w): ~800k fwd + ~800k bwd = 1.6M (one Eea pass).

Phase 4 (uncompute w_reg = dx³ and dx2_reg after Eea backward):
Eea backward already uncomputed to leave only r_low changed? No,
Eea uncomputes all its state but tx (its input) is preserved. So
...
CURRENT Eea is much cheaper than "half of 2 × Eea @ 511" — the
...
- **Path A (coset on long add chains)**: INVALIDATED — our circuit
  ...
  reached. Skip.
- **Path B (m_hist elim + karatsuba state_a_mul2)**: PARTIALLY INVALIDATED
...
The tactical path forward is **per-iter Eea cost reduction** —
...
- Narrow the step-4 tmp load from `min(n, 2n-i)` to something tighter
  based on live-bit-extents during bulk iterations (the Eea

...
| component | cost |
|---|---:|
| Eea invocation #1 (`with_kal_inv_raw`, fwd+body+bwd scale excluded) | ~1.60M |
| Eea invocation #2 | ~1.59M |
| non-Eea scaffold (muls, scale correction, FastModulo, constants) | ~0.94M |
| total | ~4.11M |
...
2. **Keep two inversions but make each ~45% cheaper.** Need per-invocation
   cost `~1.60M -> ~0.90M`, i.e. save ~700k per Eea invocation.
...
`inv(dx*bx)` merely moves the second Eea to a different denominator and adds
...
Eea DIV already spends `u+r=512` scratch, leaving 88 bits for branch cleanup.
`scratch600_sidecar_tag_bits_do_not_fix_eea_branch_recovery` gives a small
...
`unreduced_coefficient_eea_self_cleans_but_width_kills_scratch600` shows the
...
"current Eea + small tag" or "unreduced Eea".  Measuring away the high
...
makes poststate collisions rare (`curve_restricted_tagged_eea_poststate_ambiguity_is_small_but_not_exact`:
...
opening for approximate no-history Eea **only if** the reverse step can test
curve support cheaply.  It cannot do so from the Eea poststate alone:
`secp_curve_support_does_not_make_eea_branch_choice_locally_free` runs on
...
check per Eea iteration, which is not SOTA-shaped.  Do not count the rare
...
also closed: `one_lane_linear_sidecar_has_no_closed_update_for_all_eea_branches`
...
Cost at current 407/403 Eea settings:

| block | estimated Toffoli |
|---|---:|
| compute `dx^2, w=dx^3` | 250-300k |
| one Eea invocation on `w` | 1.60M |
...
## 3. Therefore the most credible Toffoli route is a cheaper Eea
...
Current Eea cost sources, across both forward and backward and both
invocations:
...
cswap + step4 cost**. This points to a jumped/windowed/divstep-style Eea,
not to local adder swaps.

## 4. Candidate structural programs

### Program A — jumped/windowed Eea (highest Toffoli relevance)
...
|---:|---:|---:|---|
| t=2 | ~2× current step | <1.2× current step | ~40% Eea win |
| t=3 | ~3× | <1.8× | ~40% win |
| t=4 | ~4× | <2.4× | ~40% win |
...
current Eea. If a low-coefficient t=2 or t=3 schedule can be synthesized
with one scratch n-register and <=~1.5n Toffoli/step equivalent, it is live.

Fresh survey from `eea_jump.rs` / scratch replay (10k inputs for the
...
A real jumped Eea must either compute the whole comparator sequence
...
Follow-up hard gate: `selected_matrix_variable_coeff_lower_bound_kills_hybrid_eea_windows`
...
row formation for the two Eea pairs costs:
...
This is above the entire current Eea invocation, not near the needed
~0.9M/invocation.  Therefore generic selected-matrix Eea windows are dead
with current arithmetic.  Any Eea-window revival must avoid quantum variable
...
Eea invocation cost. BY jump inversion is therefore the most concrete live
...
below current Eea's ~1.6M/invocation, so a live BY prototype is justified.
...
has a row-formation cost far below current Eea. The missing pieces are now
...
lead than Eea low-bit windows: no full comparator sequence, moderate matrix
...
```text
(x0,x1) -> V^-1(x0,x1)
z0      -> (z0 + e*z1) / 2^16      // one batched FastModulo shift
(z0,z1) -> U^-1(z0,z1)
```
...
uses the earlier safe Eea bulk prefix `370` under this env flag only. The
...
This is still Eea + appended BY roundtrip, not a replacement. The useful
...
normal Eea point-add, but it no longer uses a fixed odd/A trace: it copies
...
`BY_CENTERED_PAIR1_REPLACE=1`. It replaces state_a's Eea inverse and first
...
7. use the existing state_a `mul2` to zero `ty`.
...
denominator generator and Bennett replay, and it still keeps state_a `mul2` plus
all of state_b Eea. But it is the first env-gated path where a Eea-sized
...
`BY_CENTERED_PAIR2_REPLACE=1` now replaces the state_b Eea cleanup as well.
...
quotient output register. AlgoState2's `ty` product is preserved, then the ordinary
...
`BY_SCALED_PAIR2_PRODUCT_REPLACE=1` is the ground-up state_b variant that should
...
This is significantly cleaner than the centered state_b quotient-add replacement
...
Again, this is not a performance result. It proves both Eea inverse-sized
...
remove two Eea invocations  ≈ -3,169,168
...
`low_scratch_scaled_by_budget_still_beats_27m_after_state_a_mul_deletion` adds an
important correction: true tagged DIV also deletes state_a's two schoolbook
...
state_b-cleanup conceptual gap. The forward scaled BY map sends
`(0, q*x) -> (sign*q, 0)`, so the inverse map sends `(sign*q,0) -> (logical 0,
q*x)`. This is exactly the state_b operation needed after `tx=Rx-Qx` and
...
This means the full replacement is two BY replays: forward tagged DIV for state_a
and neg-r inverse product-clean BY for state_b, deleting both Eea objects and
the ordinary multiplication cleanup around them. With measured decoder margins,
`low_scratch_scaled_by_budget_still_beats_27m_after_state_a_mul_deletion` now
...
compressed-history model, plausibly cheaper than Eea but not yet a complete
...
Approximate Eea cutoff is not a shortcut under the current phase-clean
...
```text
state_a=403,state_b=403: classical=0, phase_batches=1
state_a=407,state_b=400: classical=0, phase_batches=2
state_a=400,state_b=403: classical=1, phase_batches=1
state_a=390,state_b=390: classical=16, phase_batches=49
```

So current Eea iteration counts are effectively tight unless the phase
...
1. Eea input (`dx` or `dx^3`) remains uncomputable after output mutation.
...
`old dx` and `new Rx` as independent live n-bit values after Eea backward,
it has already lost; zeroing one from the other is point-subtraction.

### Program C — Eea representation rewrite for qubits

Even a cheaper Eea will not hit 1175q with the current state layout.
Required qubit reductions:

| source | potential saving |
|---|---:|
| `m_hist` compression/elimination | 407q |
| fold Eea input copy into `tx`/scratch | 256q |
...
- a self-cleaning Eea body whose inverse branch is recoverable from end
  state, or
...
Current affine scaffold has no long non-Eea add region. Coset becomes
...
by toggling `shifted_v` under `!sign` and using `!sign` as the fast-RippleAdder
...
object.  That collapses Strategy E back to the measured state_b
...
current primitives**.  Do not wire it using existing Eea/BY product-clean
...
Forward-only coefficient-transform Eea also got a stronger exactness check.
...
`initial_x_to_branch_history_oracle_is_dense_on_toy_eea` sees branch-history
...
Kim-style unconditional Eea was checked through the same lens:
...
Coefficient-transform refinement checked: a single Eea coefficient pair
...
`a_coefficient_cancellation_is_dense_on_toy_eea` gets full-degree,
...
## 7. Post-E attempt: Eea scale absorption by denominator pre-scaling

This is a smaller structural lever than deleting an inversion, but it attacks a
measured ~206k correction-loop component.  Since raw Eea returns
`-v^-1 * 2^iters`, feed it `v' = 2^iters v`; the exposed inverse becomes exact
and the slope halving / cleanup doubling loops disappear.

AlgoState1 and state_b were wired behind `KAL_PRESCALE_PAIR1_SAFE=1` /
`KAL_PRESCALE_PAIR2_SAFE=1` using an exact RippleAdder constant prescaler and passed
the full harness:

```text
state_a: avg_toffoli=4,786,373  qubits=2,972  clean
state_b: avg_toffoli=4,771,009  qubits=2,969  clean
```
...
```text
state_a mixed:  avg_toffoli=4,223,465  qubits=2,972
state_b mixed:  avg_toffoli=4,220,405  qubits=2,969
both mixed:   avg_toffoli=4,331,952  qubits=2,972
state_a folded: avg_toffoli=4,223,465  qubits=2,969
state_b folded: avg_toffoli=4,220,405  qubits=2,965
```

The folded variants push the mixed prescale into Eea `v_w` init, removing
...
the Eea `v_w` initialization so it does not add a persistent n-bit
denominator copy.  Without that primitive, scale absorption is not SOTA-relevant.

## 8. Direct controlled-constant FastModulo correction (qubit tradeoff)

The default peak trace after the prescale work showed the global maximum at
Eea backward `bk_step6_7_8`, specifically inside modular halve's controlled
FastModulo correction.  The old `csub_nbit_const_fast` materializes the classical
...
trace with the flag enabled moves the bottleneck to Eea backward step4:
...
375 Eea bulk prefix but passes with the conservative 370 prefix:
...
small exact win in the Eea coefficient range bound.  The old
`R_SMALL_THRESHOLD=255` controlled when `r := 2r` / reverse halve could skip the
FastModulo correction because `r` was guaranteed small.  Env probes showed:

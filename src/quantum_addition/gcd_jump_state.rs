//! Classical analysis for a possible **hybrid Eea-jump** moonshot.
//!
//! Idea: keep the existing Eea cleanup machinery (`r,s,m_hist`) but try to
...
//! 1. How many distinct `P_t` appear along actual secp256k1 Eea trajectories?
...
impl KCase {
    /// Matrix for the (u, v) register update over one Eea micro-step:
    ///
    ...
    /// Matrix for the coefficient-side (r, s) update over one Eea step.
    ///
    /// Derived directly from the implemented sequence in `eea_iteration`:
    ///
...
#[inline(always)]
fn eea_case(u: U256, v: U256) -> KCase {
    if !u.bit(0) {
        KCase::UEven
    } else if !v.bit(0) {
        KCase::VEven
...
#[inline(always)]
pub(crate) fn eea_step_uv(u: U256, v: U256) -> (U256, U256, KCase) {
    match eea_case(u, v) {
...
/// Observe a t-step Eea window starting from full-width `(u, v)`.
/// Returns `(u_t, v_t, obs)`.
...
/// Sample actual secp256k1 Eea trajectories and measure the compressibility
/// of t-step local transition matrices keyed by low-w bits.
pub fn hybrid_eea_window_survey(
    seed: &[u8],
    n_inputs: usize,
    w: usize,
    t: usize,
    ...
    for input_idx in 0..n_inputs {
        if (input_idx & 31) == 0 {
            check_deadline(deadline, "eea_jump::hybrid_eea_window_survey");
        }
...
mod tests {
    ...
    #[test]
    fn selected_matrix_variable_coeff_lower_bound_kills_hybrid_eea_windows() {
        // A matrix-hint Eea window only becomes a Toffoli route if applying
        ...
        for &(t, coeff_bits) in &[(4usize, 5usize), (8, 9), (16, 17)] {
            ...
            // for the two Eea pairs and omits all selection/QROM/cleanup.
            ...
            eprintln!(
                "hybrid Eea selected-matrix lower bound: t={t}, coeff_bits={coeff_bits}, window_ccx={ccx}, windows={windows}, invocation_lower={invocation_lower}, peak={}q",
                b.peak_qubits
            );
            assert!(
                invocation_lower > 900_000,
                "selected Eea matrix route may still hit 0.9M/invocation; synthesize it"
            );
        }
    }

    #[test]
    fn hybrid_eea_window_survey_test() {
        for &(w, t) in &[(6usize, 4usize), (8usize, 4usize), (8usize, 6usize)] {
            let s = hybrid_eea_window_survey(b"hybrid-eea-window-seed-v1", 10_000, w, t);
            eprintln!("=== hybrid Eea window survey (w={}, t={}) ===", w, t);

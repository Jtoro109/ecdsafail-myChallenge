...
- **Architecture pivot, not micro-tuning:** stop spending time on Kaliski thresholds/local Solinas. New BY selector test `streaming_limb_selector_is_exact_but_state_heavy` proves a no-full-denominator 16-bit limb recurrence `(f_j,g_j)=A_j*(p>>16j,x>>16j)+c_j` can generate all 35 branch windows exactly with 12 limbs per `A/c` entry, but the naive state is `6*12*16=1152` bits (8 limbs fails on all sampled denominators). `streaming_limb_selector_folds_constant_p_column_but_still_too_large` folds the constant `p` column into carry: exact with 17 limbs at `4*17*16=1088` bits, while 16 limbs fails on all samples. `separate_width_streaming_selector_reaches_816_bits` improves this to `(2*9+17+16)*16=816` bits; 8-limb x-column and 15-limb c1 variants fail. `tail_exhausted_streaming_selector_has_528_bit_carry_core` shows that after the first 16 windows consume all quantum x bits, only a 528-bit carry core is needed (plus a temporary 288-bit x-column workspace before tail exhaustion). `first16_pattern_history_entropy_is_low_gate_but_not_low_qubit` finds first16 pattern entropy≈195.4 bits and fixed per-window IDs=208, so carry+early-history≈736 bits: plausible for 1425q low-gate if b-workspace is borrowed/recomputed, not enough for 1175q. `projective_normalized_streaming_selector_loses_high_bits` kills the simple 816-bit normalization (`c0=1`): it fails on all 64 samples because high 2-adic information is discarded. Next work: make the first-16-window 288-bit x-column workspace reversible/borrowed and avoid raw pattern history, or factor it via Hermite window structure. See `src/point_add/by_sota_architecture.md`.

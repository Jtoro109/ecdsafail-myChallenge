...
- **Architecture pivot, not micro-tuning:** stop spending time on Kaliski thresholds/local Solinas. Strongest current route is now the **full BY ratio selector**: `full_ratio_state_streams_all_branches_in_560_bits` validates that `h0=x*p^-1 mod 2^560` plus closed ratio updates streams all 560 branch bits exactly and tapers to zero. The fixed 560-qubit register can hold remaining ratio plus consumed history, fitting the ~600-bit low-qubit selector allowance without denominator pair, x-column residues, carry rows, or separate tail history. But `ratio_a_step_is_inverse_dense_and_common` shows the direct A-step is gate-dangerous: toy 16-bit A map high bit has ANF degree 14 and density 7268/32768; real traces average ~132 A-steps. `ratio_window_mobius_denominators_are_not_near_constant` says 16-step windowing helps only partly: v2(m01) hist over 2240 windows is `[0,347,673,380,317,164,104,55,49,17,18,2,2,9,9,2,92]`, so 45.5% have v2 1-2 and geometric-series inversion is still nontrivial. Full-ratio is state-optimal, not yet gate-optimal; next work is a better windowed/Möbius or inverse-maintenance circuit. Older selector results remain fallback diagnostics: folded limb recurrence needs 1088 bits; separate widths reach 816 bits; tail-only c-core is 528 bits; post-window16 ratio is 304 bits; first16 entropy≈195.4/fixed=208; truncated 9-limb x-column is not locally reversible; projective `c0=1` normalization fails. See `src/point_add/by_sota_architecture.md`.
